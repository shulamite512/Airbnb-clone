    ...
    const propertiesWithImages = await Promise.all(
      properties.map(async (property) => {
        if (isPhotosEmpty(property.photos)) {
          const images = await getPropertyImages(property.property_type || 'house', property.location);
...
router.post('/properties', authenticate, async (req, res) => {
  try {
    ...
    if (!propertyPhotos || (Array.isArray(propertyPhotos) && propertyPhotos.length === 0)) {
      const generatedImages = await getPropertyImages(property_type, location);
