...
const AI_API_URL = 'http://localhost:8000/api/agent';
...
export default function AIChatbot({ booking: propBooking }) {
  ...
  const handleGeneratePlan = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${AI_API_URL}/plan`, {
        method: 'POST',
      ...
      setMessages([
        ...messages,
        { type: 'error', text: 'Failed to generate plan. Make sure the AI service is running on port 8000.' },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      ...
      {isOpen && (
        <div className="fixed inset-0 md:inset-auto md:bottom-24 md:right-6 md:w-96 md:h-[600px] w-full h-full bg-white md:rounded-2xl shadow-2xl flex flex-col z-50 md:border">
          {/* Header */}
